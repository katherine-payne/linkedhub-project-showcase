...
export default function CompanyPage() {
  ...
  return (
    <div className="flex md:flex-row flex-col gap-4 justify-center items-center md:items-start mx-4">
      {company && company.requests.length > 0 && <div className="max-w-xl w-full md:max-w-xs bg-white shadow-md border border-border-neutral p-4 rounded-lg">
                  ...
                  <div className="flex flex-row gap-1">
                    <PrimaryButton
                      onClick={async () => {
                        ...
                        updatedCompany.requests = updatedCompany.requests.filter((r) => r._id !== recruiter._id) // remove from requests
                        updatedCompany.recruiters.push(recruiter) // add to recruiters
                        const r: Company = await updateCompany(updatedCompany)
